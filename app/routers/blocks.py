...
async def get_all_blocks(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve blockchain data: {str(e)}"
        )
...
async def get_block(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve block: {str(e)}"
        )
...
async def mine_block(
    ...
    try:
        if not blockchain.pending_transactions:
            raise HTTPException(
                status_code=400, detail="No pending transactions to mine."
            )

        block = blockchain.mine_block(miner_address)
        if not block:
            return api_response(
                data={"block": None}, message="No block to mine.", status_code=200
            )
